...
<body>
    ...
    <div class="container">
        <p><a class="btn" href="javascript:history.back()">&larr; Retour</a></p>
        <?php if (!$employee) { ?>
            <h1>Employé introuvable</h1>
        <?php } else { ?>
            <h1><?= $employee['first_name'] ?> <?= $employee['last_name'] ?></h1>
            <p><a href="change_dept.php?emp_no=<?= urlencode($employee['emp_no']) ?>">
                    <button class="btn btn-secondary" type="button">Changer de département</button>
                </a></p>
            <p><a href="become_manager.php?emp_no=<?= urlencode($employee['emp_no']) ?>">
                    <button class="btn btn-secondary" type="button">Devenir Manager</button>
                </a></p>
            <p><a href="emp_form.php?emp_no=<?= urlencode($employee['emp_no']) ?>">
                    <button class="btn btn-secondary" type="button">Modifier l'employé</button>
                </a></p>
            <table class="table" border="1">
                ...
                <tr>
                    ...
                    <td><?= isset($employee['salary']) ? number_format($employee['salary'], 0, ',', ' ') . ' €' : '—' ?></td>
                </tr>
                <tr>
